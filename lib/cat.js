...
class El {
    ...
    getAllKeys (...rest) {                   // getAllKeys(obj)
        if(typeof rest[0] != 'object' || rest.length != 1){
            throw new Error('wrong! you may try like this  => getAllKeys(object)')
        };
        let arr;
        let keys = Object.keys(rest[0]);
        let symbols = Object.getOwnPropertySymbols(rest[0]);
        return arr = keys.concat(symbols);
    }

    iterable (...rest) {                    // iterable(obj)
        if(typeof rest[0] != 'object' || rest.length != 1){
            throw new Error('wrong! you may try like this  => iterable(object)')
        };
        ...
        return {
            next: () => {
                const name = names[i++];
                const val = this[name];
                return { value: { name, val }, done: i > length } 
               }

            }
        } 

        rest[0][Symbol.iterator] = iteratorFun;
    }
}
